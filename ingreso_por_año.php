...
if ($conn->connect_error) {
    ...
    if ($usuario_id === 0) {
        ...
        // Calcular la suma de los montos de ingresos por año
        $sql_sum = "SELECT YEAR(fecha_ingreso) AS year, SUM(monto) AS total FROM ingresos WHERE id_usuarios = ? GROUP BY year";
        ...
        if ($result_sum !== false && $result_sum->num_rows > 0) {
            ...
            $response["message"] = "Suma de ingresos por año obtenida con éxito";
            $response["data"] = $rows_sum;
        } else {
            $response["message"] = "Error al obtener la suma de ingresos por año.";
        }
    }
}
